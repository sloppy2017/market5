...
@Service
public class Data_1Day extends TaskServiceBase{
  ...
  public void task() {
    logger.debug("go Task 1 Day!" + DateUtil.getCurrentTimestamp());
    scheduled(INTERVAL_TIME_SECONDS,CACHE_LOCK, TimeUnit.SECONDS);
  }

  @Override
  public void doJob(int INTERVAL_TIME_SECONDS) {
    logger.debug("go Task 1 Day lock!" + DateUtil.getCurrentTimestamp());
    ...
    for(String tradePair: list){
      ...
      new Thread(new KLineDataThread(stringRedisTemplate,matchMoneyMapper,
        "1DAY",getBeginTime_Month(1),getEndTime_Month(),tradePair)).start();
    }
  }
}
