...
@Service
public class DataService_1Month extends TaskServiceBase{

  public static String CACHE_LOCK = "cache.lock.1month";

  private static String MINUTES_PATH_NAME = "1MONTH/";

  private static int INTERVAL_TIME_SECONDS = 5;

  @Scheduled(cron = "0 0 1 1 1/1 ?")
  public void task() {
    logger.debug("go Task 1 Month!");
    if(setex(CACHE_LOCK, DateUtil.getCurrentTimestamp()+"")){
      logger.debug("get 1 Month lock!" + DateUtil.getCurrentTimestamp());
      stringRedisTemplate.expire(CACHE_LOCK,INTERVAL_TIME_SECONDS,TimeUnit.SECONDS);
      doJob();
    }else {
      logger.debug("NOT get 1 Month lock!");
    }
  }
  ...
  public void doJob() {
    ...
    for(String tradePair: list){
      ...
      new Thread(new RealTimeBlockDataThread(tradePair,filePath + MINUTES_PATH_NAME,
        this.getPahtEnd("/yyyy/MM"),getBeginTime_Month(1),getEndTime_Month(), FILE_SUFFIX, matchMoneyMapper)).start();
    }
  }
}
