...
import com.c2b.coin.account.feign.SANClient;
...
@Service
public class SANCWithDrawAndDeposit implements IDigitalCoinWithDrawAndDeposit{

	@Autowired
	private SANClient sanClient;
	@Value("${wallet.sancWithDrawPoundage}")
	private String fee;
	@Value("${wallet.sancWithDrawLimit}")
	private String limit;
	@Value("${wallet.sancWithDrawMax}")
	private String max;
	...
	public String noticeWithDrawService(String currencyName, AssetLog assetLog) {
		...
		AjaxResponse ajaxResponse=this.sanClient.sendMoney(withdrawLog);
		if(ajaxResponse==null) {
			return null;
		}
		if(!ajaxResponse.isSuccess()||null==ajaxResponse.getData()) {
			throw new AssetChangeException(ERROR_Map.get(ajaxResponse.getError().getCode()), "通知三界宝钱包失败");
		}
	...
	public String getAddress(String userName) {
		AjaxResponse ajaxResponse=this.sanClient.createWallet(userName);
		String address =ajaxResponse.getData().toString();
		return address;
	}
	...
	public boolean checkAmountMax(Long userId, BigDecimal amount) {
		...
		List<AssetLog> list = assetLogMapper.findByCurrency(AssetChangeConstant.WITHDRAW,userId, 4,startDate, endDate);
