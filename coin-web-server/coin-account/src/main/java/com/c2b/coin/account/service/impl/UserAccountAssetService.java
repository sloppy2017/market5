...
public class UserAccountAssetService implements IUserAccountAssetService {
	...
	@Override
	public List<AccountAssetVO> getTotalAsset(long userId) {
		//1. 从数据库获取用户全部资产
		List<AccountAssetVO> accountAssetVOList = this.userAccountMapper.selectAccountAssetVO(userId);
		return accountAssetVOList;
	}
	...
	@Override
	public AssetTotalVO assetTotal(long userId) {
		//计算总资产
		UserAccount userAccount = new UserAccount();
		userAccount.setUserId(userId);
