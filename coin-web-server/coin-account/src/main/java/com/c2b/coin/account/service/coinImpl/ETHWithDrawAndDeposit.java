...
import com.c2b.coin.account.feign.ETHClient;
...
@Service
public class ETHWithDrawAndDeposit implements IDigitalCoinWithDrawAndDeposit{

	@Value("${wallet.ethWithDrawPoundage}")
	private String fee;
  @Value("${wallet.ethWithDrawLimit}")
  ...
  static {
	...
	public String noticeWithDrawService(String currencyName, AssetLog assetLog) {
		...
		AjaxResponse ajaxResponse=this.ethClient.sendMoney(withdrawLog);
	...
	public String getAddress(String userName) {
		AjaxResponse ajaxResponse=this.ethClient.createWallet(userName);
		String address =null;
		if(!ajaxResponse.isSuccess()) {
			ajaxResponse=this.ethClient.getAddress(userName);
		}
