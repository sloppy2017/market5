package com.dynamo.message.util;
...
public class RestClient {
  ...
  public <T> T postEMay(String url, String appId, byte[] content){
    HttpHeaders httpHeaders = new HttpHeaders();
    httpHeaders.add("appId", appId);
    httpHeaders.add("gzip", "on");
    httpHeaders.add("encode", "UTF-8");
    HttpEntity<byte[]> formEntity = new HttpEntity<byte[]>(content, httpHeaders);
    ResponseEntity<byte[]> responseEntity = restTemplate.postForEntity(url, formEntity, byte[].class);
    return null;
  }
