...
public class WebSocketWatcher implements Runnable {
    private static final Logger logger = LogManager.getLogger(WebSocketWatcher.class);

    @Override
    public void run() {
        logger.info("Start to watch ws servers.");
        Collection<WSClient> clients = CoralWebSocketClient.clients();
        for (WSClient client : clients) {
            if (client.getStatus() == 0) {
                long requestId = RandomUtils.nextLong(0, 100L);
                CoralWebSocketClient.queues.put(requestId, new LinkedBlockingQueue<>(1));
                try {
...
//                        channel.close();
                    ...
                    if(result != null){
                        ...
                        if(json.has("status") && json.getString("status").equals("success")){
                            client.setStatus(1);
                            CoralWebSocketClient.updateClient(client);
                        ...
                        if(client.getTimestamp() > 120000) {
                            CoralWebSocketClient.initServer(client.getType(), client.getUri());
                        }
                    }
                } catch (Exception e) {
