 ...
 * <ul>
 * <li>org.sanchain.client.jsonrpc4j.async.socket.timeout - overall socket idle
 * (keep-alive) timeout in milliseconds, default is 30 seconds</li>
 * <li>org.sanchain.client.jsonrpc4j.async.connect.timeout - socket connect timeout
 * in milliseconds, default is 30 seconds</li>
 * <li>org.sanchain.client.jsonrpc4j.async.socket.buffer - socket buffer size in
 * bytes, default is 8Kb (8192 bytes)</li>
 * <li>org.sanchain.client.jsonrpc4j.async.tcp.nodelay - true to use TCP_NODELAY,
 * false to disable, default is true
 * <li>org.sanchain.client.jsonrpc4j.async.max.inflight.route - maximum number of
 * in-flight requests per route (unique URL, minus query string), default is 500
 * </li>
 * <li>org.sanchain.client.jsonrpc4j.async.max.inflight.total - maximum number of
 * total in-flight requests (across all providers), default is 500</li>
 * <li>org.sanchain.client.jsonrpc4j.async.reactor.threads - number of asynchronous
...
public class JsonRpcHttpAsyncClient {
	...
	private void writeRequest(String methodName, Object arguments,
		...
		if (arguments != null && arguments.getClass().isArray()) {
			...
			if (args.length > 0) {
				request.put("params",
                        mapper.<JsonNode>valueToTree(Object[].class.cast(arguments)));
			}

			// collection args
		} else if (arguments != null && Collection.class.isInstance(arguments)) {
			if (!Collection.class.cast(arguments).isEmpty()) {
				request.put("params", mapper.<JsonNode>valueToTree(arguments));
			}
	...
	private void initialize() {
		...
		params.setIntParameter(CoreConnectionPNames.SO_TIMEOUT, Integer
				.getInteger("org.sanchain.client.jsonrpc4j.async.socket.timeout",
						30000));
		params.setIntParameter(CoreConnectionPNames.CONNECTION_TIMEOUT, Integer
				.getInteger("org.sanchain.client.jsonrpc4j.async.connect.timeout",
						30000));
		params.setIntParameter(CoreConnectionPNames.SOCKET_BUFFER_SIZE, Integer
				.getInteger("org.sanchain.client.jsonrpc4j.async.socket.buffer",
						8 * 1024));
		params.setBooleanParameter(CoreConnectionPNames.TCP_NODELAY, Boolean
				.valueOf(System.getProperty(
						"org.sanchain.client.jsonrpc4j.async.tcp.nodelay", "true")));
		...
		try {
			...
			config.setIoThreadCount(Integer.getInteger(
					"org.sanchain.client.jsonrpc4j.async.reactor.threads", 1));
		...
		pool.setDefaultMaxPerRoute(Integer.getInteger(
				"org.sanchain.client.jsonrpc4j.async.max.inflight.route", 500));
		pool.setMaxTotal(Integer.getInteger(
				"org.sanchain.client.jsonrpc4j.async.max.inflight.total", 500));
