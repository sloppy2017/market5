...
public class Seed {
    ...
    public static IKeyPair createKeyPair(byte[] seedBytes, int accountNumber) {
        if(seedBytes.length != 16)
            throw new CoralException("seedBytes MUST be 16 bytes: " + Hex.toHexString(seedBytes));
    ...
    public static IKeyPair createKeyPair(byte[] seedBytes) {
        if(seedBytes.length != 16)
            throw new CoralException("seedBytes MUST be 16 bytes: " + Convert.bytesToHex(seedBytes));
    ...
    public static byte[] genSeedFromBtcPriv(byte[] priv) {
        if(priv.length != 32) throw new CoralException("BTC private key MUST be 32 bytes!");
