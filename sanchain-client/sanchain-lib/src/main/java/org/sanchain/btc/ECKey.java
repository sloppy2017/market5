...
import org.sanchain.btc.account.Address;
...
import static org.sanchain.btc.BitUtil.bytesToHexString;
import static org.sanchain.btc.BitUtil.parseAsHexOrBase58;
...
public class ECKey implements Serializable {
    ...
    */
     ...
     */
    public static ECKey getECFromPrivStr(String privStr) throws CoralException {
        if(privStr == null || privStr.length() < 32) throw new CoralException("Wrong private key : " + privStr);
        byte[] priv;
        ...
        if(len == 44 && privStr.endsWith("=")) {//maybe Base64
            ...
            if(src.length != 37 || src[0] != (byte)0x80)
                throw new CoralException("Wrong private key of Btc-Qt format: " + bytesToHexString(src));
            priv = new byte[32];
            System.arraycopy(src, 1, priv, 0, 32);
        } else if(len == 64) {          //maybe HEX
            priv = parseAsHexOrBase58(privStr);
        } else {
            throw new CoralException("Unknown private key format: " + privStr);
        }
    ...
    /**
     * Exports the private key in the form used by the Satoshi org.sanchain.client "dumpprivkey" and "importprivkey" commands. Use
