...
import org.sanchain.core.Utils;
...
public class B58 {
    ...
    public static final String DEFAULT_ALPHABET = "sprhnaf39wBUDNEGHJKLM4PQRST7VWXYZ2bcdeCg65jkm8oFqi1tuvAxyz";
    ...
    public byte[] decode(String input) throws CoralException {
        if (input.length() == 0) {
            return new byte[0];
        }
        ...
        for (int i = 0; i < input.length(); ++i) {
            ...
            if (digit58 < 0) {
                throw new CoralException("Illegal character " + c + " at " + i);
            }
    ...
    public BigInteger decodeToBigInteger(String input) throws CoralException {
     ...
     *
     * @throws org.sanchain.core.exception.CoralException if the input is not baseFields 58 or the checksum does not validate.
     */
    public byte[] decodeChecked(String input, int version) throws CoralException {
        byte buffer[] = decode(input);
        if (buffer.length < 4)
            throw new CoralException("Input too short");
        byte actualVersion = buffer[0];
        if (actualVersion != version) {
            throw new CoralException("Bro, version is wrong yo:" + input + " ver=" + actualVersion + " need=" + version);
        }
        ...
        if (!Arrays.equals(checksum, hashed))
            throw new CoralException("Checksum does not validate");
    ...
    /** family seed */
    public byte[] decodeFamilySeed(String seed) throws CoralException {
    ...
    /** Addrress */
    public byte[] decodeAddress(String address) throws CoralException {
    ...
    /** Node public (by Fau) */
    public byte[] decodeNodePublic(String np) throws CoralException {
    ...
    public byte[] decodeAccountPublic(String ap) throws CoralException {
