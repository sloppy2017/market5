...
import org.sanchain.btc.BitUtil;
import org.sanchain.core.exception.CoralException;
import org.sanchain.core.hash.RFC1751;
...
public class Wallet {
    ...
    public static Wallet fromSeedString(String seed) {
        if (seed == null || seed.length() < B58.LEN_FAMILY_SEED - 2 || !seed.startsWith("r"))
            throw new CoralException("AccountID.fromSeedString() param ERROR: " + seed);
        byte[] b16 = b58.decodeFamilySeed(seed);
        if (b16.length != 16) throw new CoralException("decodeFamilySeed() not 16 bytes: " + Hex.toHexString(b16));
