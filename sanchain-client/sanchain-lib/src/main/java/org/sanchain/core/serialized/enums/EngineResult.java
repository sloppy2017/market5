...
public enum EngineResult implements SerializedType
{
    ...
    temDISABLED(-272, "The transaction requires logic that is currently disabled."),
    temBAD_DIV_TYPE(-271, "Bad dividend type"),
    temBAD_RELEASE_SCHEDULE(-270, "Malformed: Bad ReleaseSchedule"),
    temBAD_SIGNER(-269, "Malformed: No signer may duplicate account or other signers."),
    temBAD_QUORUM(-268, "Malformed: Quorum is unreachable."),
    temBAD_WEIGHT(-267, "Malformed: Weight must be a positive value."),

    temUNCERTAIN (-266, "In process of determining result. Never returned."),
    temUNKNOWN (-265, "The transactions requires logic not implemented yet."),
    ...
    tefREFEREE_EXIST(-185, "This account has already had a referee."),
    tefREFERENCE_EXIST(-184, "This account has already had a reference."),
    tefBAD_SIGNATURE(-183, "A signature is provided for a non-signer."),
    tefBAD_QUORUM(-182, "Signatures provided do not meet the quorum."),
    tefNOT_MULTI_SIGNING(-181, "Account has no appropriate list of multi-signers."),
    tefBAD_AUTH_MASTER(-180, "Auth for unclaimed account needs correct master key."),
    ...
    tecOVERSIZE(145, "An internal error has occurred during processing.");
    ...
    public static class Translator extends TypeTranslator<EngineResult> {
        ...
        public EngineResult fromParser(BinaryParser parser, Integer hint) {
            byte data = parser.read(1)[0];
            int code = data & 0xFF;
            EngineResult result = fromInteger(code);
            return result;
        }
